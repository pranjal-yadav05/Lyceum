  ...
  useEffect(() => {
    ...
    const initializePeer = () => {
      ...
      peer.current.on("call", async (call) => {
        ...
        const callerMediaState = call.metadata?.mediaState || {
          video: true,
          audio: true,
        };
    ...
    const initializeMedia = async () => {
      try {
        ...
        socket.current.on(
          "media-state-changed",
          ({ userId, mediaState: newMediaState }) => {
            setPeers((prevPeers) =>
              prevPeers.map((peer) => {
                if (peer.id === userId) {
                  return {
  ...
  return (
    <div className="min-h-screen bg-[#0f0a1f] text-white p-4">
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-4 h-[calc(100vh-200px)]">
        <div
          className={`lg:col-span-3 overflow-auto ${
