...
import { Share2, Edit2, Clock, Users, Flame, ArrowLeft, Upload, UserPlus, UserMinus } from 'lucide-react';
...
const ProfilePage = () => {
  ...
  const checkFriendRequestStatus = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/friends/request-status/${currentUsername}/${username}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="relative mb-8">
            ...
            <div className="h-48 md:h-64 rounded-lg overflow-hidden relative">
              ...
              />
              {isCurrentUser && (
              <label htmlFor="coverImage" className="absolute top-2 right-2 cursor-pointer bg-black/50 hover:bg-black/70 p-2 rounded-full transition-all">
                {isUploading ? <LoadingSpinner /> : <Upload className="h-6 w-6 text-white" />}
                <input
                  type="file"
                  id="coverImage"
                  className="hidden"
            ...
            <div className="absolute bottom-0 left-4 transform translate-y-1/2 flex items-end">
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-[#0f0a1f] overflow-hidden bg-[#1a1425] relative group">
                ...
                />
                { isCurrentUser && (
                <label htmlFor="profileImage" className="absolute inset-0 cursor-pointer bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                  {isUploading ? <LoadingSpinner /> : <Upload className="h-8 w-8 text-white" />}
                  <input
                    type="file"
                    id="profileImage"
                    className="hidden"
            ...
            {!isCurrentUser && (
                <Button 
                  variant="outline" 
                  className={`${friendRequestSent ? 'bg-red-600 hover:bg-red-700' : 'bg-purple-600 hover:bg-purple-700'}`}
