...
router.get('/friends/:username', authenticateToken, async (req, res) => {
  ...
  try {
    const user = await User.findOne({ username }).populate('friends', 'username profileImage');
...
router.get('/request-status/:senderId/:receiverId', authenticateToken, async (req, res) => {
